...
/**
 * @brief Standard About dialog provided by the MFC wizard.
 */
...
/**
 * @brief Constructs the main dialog object, initializing default variables and the screen light module.
 * @param pParent Pointer to the parent window, if any.
 */
CFacialAttendance2026Dlg::CFacialAttendance2026Dlg(CWnd* pParent)
...
/**
 * @brief Binds UI control resources to internal class member variables.
...
/**
 * @brief Enumerates connected DirectShow video input devices.
 * @return A vector of strings containing the friendly names of the cameras.
 */
...
/**
 * @brief Core initialization routine executing upon dialog creation.
 *        Configures the UI elements, initializes modules, and starts worker threads.
 * @return TRUE indicating initialization completed.
 */
...
/**
 * @brief Toggles the enabled state of all user input text fields and buttons for records.
...
/**
 * @brief Retrieves system cameras and populates the camera selector combo box.
 */
...
/**
 * @brief Sets up the available facial identification algorithm options for the user interface.
 */
...
/**
 * @brief Spawns the background worker thread responsible for capturing frames and performing analysis.
 */
...
/**
 * @brief Configures application coordinate metrics updating font references with appropriately scaled heavy strokes explicitly.
 */
...
/**
 * @brief Restricts user-editable areas applying specialized font styling defining safe constraints systematically.
 */
...
/**
 * @brief Constructs external list visual parameters aligning tracking columns formatting lists seamlessly.
 */
void CFacialAttendance2026Dlg::InitializeListControl()
{
    ...
    if (pListCtrl != nullptr) {
        ...
        pListCtrl->InsertColumn(0, _T("*"), LVCFMT_LEFT, 17);
        pListCtrl->InsertColumn(1, _T("TIME"), LVCFMT_LEFT, 45);
        pListCtrl->InsertColumn(2, _T("NAME"), LVCFMT_LEFT, 115);
        pListCtrl->InsertColumn(3, _T("ID"), LVCFMT_LEFT, 70);
    }
}

/**
 * @brief Recalls cached lighting system options resetting interface values matching user expectations safely.
 */
...
/**
 * @brief Asserts primary application behaviors pulling cached configurations effectively.
 */
...
/**
 * @brief Catches generic application commands applying specialized handlers when bounds dynamically shape.
 * @param nID Sent message identification standard format tracking actions reliably.
 * @param lParam System linked mapping defining parameter states correctly.
 */
...
/**
 * @brief Assures graphical consistency refreshing window layouts automatically upon system cues seamlessly.
 */
...
/**
 * @brief Supplies appropriate cursor iconography establishing reliable graphical representation reliably.
 * @return Safely extracted handle mapping system icon structures natively visually.
 */
...
/**
 * @brief Sits listening bridging main tracking updates flushing latest buffered components dynamically clearly.
 * @param nIDEvent Timer identifier matched intrinsically evaluating sequence.
 */
...
/**
 * @brief Evaluates generated tracking elements and writes buffered OpenCV layers natively inside visual canvas windows.
 */
 ...
 * @param mat Encoded color matrix metric values defined intuitively.
...
void CFacialAttendance2026Dlg::OnBnClickedClose()
{
 ...
 /**
  * @brief Clears resources managing thread synchronization states avoiding memory leaks.
  */
 ...
 * @param wParam Custom defined RGB message block securely cast.
...
/**
 * @brief Initializes the screen light mode safely and applies the current setting.
 * @param wParam Unused parameter.
 * @param lParam Unused parameter.
 * @return Always returns 0.
 */
LRESULT CFacialAttendance2026Dlg::OnInitScreenLight(WPARAM wParam, LPARAM lParam)
{
...
/**
 * @brief Shrinks capture extents unifying representations efficiently.
 * @param inOutFrame Reference camera output block correctly.
 * @param outResizedFrame Matched processing array.
 * @param outDisplayFrame Visual formatting.
 */
...
/**
 * @brief Asserts detection boundaries filtering out faces not residing logically near visual center points using Haar cascades.
 ...
 * @param resizedFrame Main tracking area mapping physical light bounds.
 * @param faces Found face areas limiting evaluation.
 */
...
/**
 * @brief Commits diagnostic frame rates strings visibly validating ongoing computational states.
 * @param memDC Target drawing interface device contexts.
 * @param x Horizontal corner offset mapped.
...
/**
 * @brief Constructs framing overlays assisting human alignment explicitly gracefully and accurately.
 * @param memDC Interface mapping active windows.
 * @param x Baseline offset defining horizontal spans.
 * @param y Height baseline correctly predictably efficiently effectively.
 * @param w Bounding extent logically tracking completely.
 * @param h Vertical tracking natively.
 */
...
/**
 * @brief Draws a warning message on the screen indicating that the user should face the center.
 * @param memDC Render device context.
 * @param x Horizontal origin for centering.
...
/**
 * @brief Converts a percentage value (0-100) from the slider to a corresponding grayscale COLORREF.
...
/**
 * @brief Applies the selected screen light mode (Slider, Auto, or None) and updates the module.
 * @param mode The target screen light mode.
 */
...
/**
 * @brief Applies a specific brightness value from the slider control.
 * @param value Brightness scalar metric.
 */
...
/**
 * @brief Handles the radio button to set the screen light to manual slider mode.
 */
...
/**
 * @brief Handles the radio button to set the screen light to automatic mode.
 */
...
/**
 * @brief Handles the radio button to disable the screen light module.
 */
...
/**
 * @brief Confirms the currently extracted identification info and adds it to the attendance list log.
 */
 ...
 * @param strTime Timestamp of the attendance record.
 * @param strName Recognized or fully verified user name.
 * @param strID System identity code for the user.
 * @param strComment Optional descriptive note.
 */
...
/**
 * @brief Handles slider UI tracking events to intelligently update brightness visually.
 * @param nSBCode Scroll bar event code.
 * @param nPos Tracking offset limits internally.
 * @param pScrollBar Pointer to the scroll bar instance.
 */
...
/**
 * @brief Menu handler explicitly cleanly exiting the application.
 */
...
/**
 * @brief Menu handler to display the current attendance tracking array log visually.
 */
...
/**
 * @brief Prepares menu commands UI appropriately and accurately before rendering.
 * @param pPopupMenu Menu reference to operate on.
 * @param nIndex Tracking numerical positional list.
...
/**
 * @brief Initiates logic logging operational durations effectively, opening detection metric folders.
 */
...
/**
 * @brief Flushes accumulated matrices tracking detection operational cycles cleanly directly tracking latency limits.
 * @param currentMethod Face detection context evaluating.
 */
...
/**
 * @brief Calculates the sharpness of an image using the variance of the Laplacian.
 * @param img The input image matrix.
 * @return A double value representing the sharpness score.
 */
...
/**
 * @brief Calculates the contrast of an image using the standard deviation of pixel intensities.
 * @param img The input image matrix.
 * @return A double value representing the contrast score.
 */
...
/**
 * @brief Calculates a composite quality score for a detected face.
 * @param sharpness The calculated sharpness metric.
 * @param faceConfidence The confidence score returned by the face detector.
 * @param contrast The calculated contrast metric.
 * @return A weighted total score evaluating the overall quality of the face image.
 */
...
/**
 * @brief Draws a cv::Mat image onto a static UI control, resizing it to fit while maintaining aspect ratio.
 * @param nID The resource ID of the target static control.
 * @param mat The cv::Mat image to draw.
 */
...
/**
 * @brief Stops the camera capture, evaluates the buffered faces, and identifies the best face to display.
 */
...
/**
 * @brief Clears the existing face buffer, prepares UI inputs, and triggers the active camera capture process.
 */
void CFacialAttendance2026Dlg::OnBnClickedButtonCameraOn()
{
    ...
    m_fpsCounter.reset();
    m_bCapturing = true;
    m_mirrorView = AfxGetApp()->GetProfileInt(REG_SECTION_SETTINGS, REG_KEY_OPTION_MIRROR_VIEW, DEF_KEY_OPTION_TRUE);

    m_pauseCV.notify_one();
}

/**
 * @brief Selects all text in the name combo box when it receives focus.
 */
...
/**
 * @brief Selects all text in the ID edit control when it receives focus.
 */
...
/**
 * @brief Selects all text in the comment edit control when it receives focus.
 */
 ...
 * @param maxWidth Maximum allowed width(frame boundary).
 * @param maxHeight Maximum allowed height(frame boundary).
...
cv::Rect CFacialAttendance2026Dlg::GetPaddedRectAndShiftLandmarks(int cx, int cy, int padSide, int maxWidth, int maxHeight, const std::vector<float>& originalData, std::vector<float>& outShiftedData)
{
    ...
    outShiftedData[0] -= padX;
    outShiftedData[1] -= padY;

    for (size_t i = 4; i < 14; i += 2) {
        outShiftedData[i] -= padX;
        outShiftedData[i + 1] -= padY;
    }
...
/**
 * @brief Handles changing the face detection mode via the UI combobox.
 */
...
/**
 * @brief Handles changing the face identification mode via the UI combobox.
 */
...
/**
 * @brief Opens the evaluation results folder for face detection latency.
 */
...
/**
 * @brief Opens the evaluation results folder for face identification latency.
 */
 ...
 */
void CFacialAttendance2026Dlg::OnFileSettings()
{
    SettingDlg dlg;
